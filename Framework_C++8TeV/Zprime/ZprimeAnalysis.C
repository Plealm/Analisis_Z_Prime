		...
		if(goodjet_n >= 4)
		  {
			...
			if(met_et > 30000.)
			  {
			    ...
			    if(Lepton1_MeT_MT > 30000.)
			      {
                   // TLorentzVector definitions for jets
            ...
            TLorentzVector TotalMomentum = TLorentzVector();
            TLorentzVector twojets = TLorentzVector();
            TLorentzVector threejets = TLorentzVector();        



            Jet1.SetPtEtaPhiE(jet_pt[0], jet_eta[0], jet_phi[0],jet_E[0]);
            Jet2.SetPtEtaPhiE(jet_pt[1], jet_eta[1], jet_phi[1],jet_E[1]);
            Jet3.SetPtEtaPhiE(jet_pt[2], jet_eta[2], jet_phi[2],jet_E[2]);
            Jet4.SetPtEtaPhiE(jet_pt[3], jet_eta[3], jet_phi[3],jet_E[3]);        
            
            TotalMomentum = Jet1 + Jet2 + Jet3 + Jet4 + Lepton_1 + MeT;
            twojets = Jet1 + Jet2;
            threejets = Jet1 + Jet2 + Jet3;        
                    
            float InvMassZprime = TotalMomentum.Mag();
            float twojetsmass = twojets.Mag();
            float threejetsmass = threejets.Mag();        
				...
				TString histonames_of_global_variable[]={"hist_vismass","hist_etmiss","hist_vxp_z","hist_pvxp_n", "hist_mt", "hist_mzp","hist_m2jets","hist_m3jets"};
...
void ZprimeAnalysis::Terminate()
{
  ...
  name="output_Zprime/"+name+".root";

  const char* filename = name.c_str();

  TFile physicsoutput_Zprime(filename,"recreate");
  WriteHistograms();
  physicsoutput_Zprime.Close();
