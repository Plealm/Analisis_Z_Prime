...
std::string histname = "hist_n_jets";
float lumi = 1000.;
...
//  h_err->Add(h_signal);
  ...
  cout<<"Events ttbar: "<<sEntries_ttbar<<endl;

  std::ostringstream Entries_singletop;
  Entries_singletop << setprecision (5) << h_singletop->Integral(0,h_singletop->GetNbinsX()+1);
  std::string sEntries_singletop = Entries_singletop.str();
  cout<<"Events singletop:"<<sEntries_singletop<<endl;

  std::ostringstream Entries_h_zjets;
  Entries_h_zjets << setprecision (5) << h_zjets->Integral(0,h_zjets->GetNbinsX()+1);
  std::string sEntries_h_zjets = Entries_h_zjets.str();
  cout<<"Events zjets:"<<sEntries_h_zjets<<endl;

  std::ostringstream Entries_h_wjets;
  Entries_h_wjets << setprecision (5) << h_wjets->Integral(0,h_wjets->GetNbinsX()+1);
  std::string sEntries_h_wjets = Entries_h_wjets.str();
  cout<<"Events wjets:"<<sEntries_h_wjets<<endl;

  std::ostringstream Entries_h_drellyan;
  Entries_h_drellyan << setprecision (5) << h_drellyan->Integral(0,h_drellyan->GetNbinsX()+1);
  std::string sEntries_h_drellyan = Entries_h_drellyan.str();
  cout<<"Events drellyan:"<<sEntries_h_drellyan<<endl;

  std::ostringstream Entries_h_db;
  Entries_h_db << setprecision (5) << h_db->Integral(0,h_db->GetNbinsX()+1);
  std::string sEntries_h_db = Entries_h_db.str();
  cout<<"Events db:"<<sEntries_h_db<<endl;
  ...
  leg->AddEntry(h_zjets,("Z:"+sEntries_h_zjets).c_str(),"f");
  leg->AddEntry(h_wjets,("W:"+sEntries_h_wjets).c_str(),"f");
  leg->AddEntry(h_drellyan,("Drell-Yan:"+sEntries_h_drellyan).c_str(),"f");
  leg->AddEntry(h_db,("Diboson:"+sEntries_h_db).c_str(),"f");
...
//  leg->AddEntry(h_signal,("Z'(" + signalmass + " GeV)#rightarrow t#bar{t} x 10^{3}").c_str(),"l");
  ...
  else can->SaveAs(("Plots/"+outfile+".pdf").c_str());
}
